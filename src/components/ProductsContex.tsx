...
import { Action } from "./IncreaseDecrease";
...
export interface IStoreContext {
  ...
  saveQuantity: (item: IProduct, count: number, action: Action) => void;
  clearCart: () => void;
...
export const StoreContextProvider: React.FC = ({ children }) => {
    ...
    const addToCart = React.useCallback((item: IProduct) => {
      ...
      const saveQuantity = React.useCallback((item: IProduct, count:number, action: Action) => {
    ...
    return ( 
        <StoreContext.Provider value={{products , savedLocalStorage, saveQuantity, addToCart,deleteFromCart,addedProducts,clearCart}} >
            { children }
